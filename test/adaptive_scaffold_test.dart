...
import 'package:adaptive_scaffold/src/adaptive_scaffold/adaptive_scaffold.dart';
import 'package:adaptive_scaffold/src/adaptive_scaffold/scaffold_config.dart';
...
void main() {
  testWidgets(
    'adaptive scaffold lays out slots as expected',
    (final WidgetTester tester) async {
      final Finder smallBody = find.byKey(const SlotKey('smallBody'));
      final Finder body = find.byKey(SlotID.body);
      final Finder largeBody = find.byKey(const SlotKey('largeBody'));
      final Finder smallSBody = find.byKey(const SlotKey('smallSBody'));
      final Finder sBody = find.byKey(const SlotKey('sBody'));
      final Finder largeSBody = find.byKey(const SlotKey('largeSBody'));
      final Finder bottomNav = find.byKey(SlotID.bottomNavigation);
      final Finder primaryNav = find.byKey(SlotID.primaryNavigation);
      final Finder primaryNav1 = find.byKey(
  ...
  testWidgets(
    'adaptive scaffold animations work correctly',
    (final WidgetTester tester) async {
      final Finder b = find.byKey(SlotID.body);
      final Finder sBody = find.byKey(const SlotKey('sBody'));
  ...
  testWidgets(
    'adaptive scaffold animations can be disabled',
    (final WidgetTester tester) async {
      final Finder b = find.byKey(SlotID.body);
      final Finder sBody = find.byKey(const SlotKey('sBody'));
    ...
    (final WidgetTester tester) async {
      ...
      await Future.forEach(
        ...
        (final SimulatedLayout region) async {
          ...
          selectedIndex =
              (selectedIndex + 1) % TestScaffold.destinations.length;
          ...
          final TestScaffoldState state = tester.state<TestScaffoldState>(
            scaffold,
          );
            ...
            child: AdaptiveScaffold(
              config: AdaptiveScaffoldConfig(
                drawerConfig: const AdaptiveDrawerConfig(
                  breakpoint: testBreakpoint0_800,
                ),
...
/// [PreferredSizeWidget] is used as [AdaptiveScaffoldConfig.appBar] parameter
/// instead of [AppBar].
